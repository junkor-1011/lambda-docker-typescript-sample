...
```bash
ts-img$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
ts-img$ sam local start-api
ts-img$ curl http://localhost:3000/
```
...
```bash
ts-img$ sam logs -n HelloWorldFunction --stack-name ts-img --tail
```
...
```bash
aws cloudformation delete-stack --stack-name ts-img
```
